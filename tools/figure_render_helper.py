#!/usr/bin/env python3
"""Figure-render helper for the teaser / AI-generated-figure workflow.

Self-contained companion to references/image-render-invocation.md. Provides
three subcommands around a Codex-driven raster render:

  preflight  -- check that the `codex` CLI app-server is reachable
  finalize   -- promote the accepted PNG to figure_final.png + emit LaTeX/log
  verify     -- confirm the final artifacts were written correctly

Only the Python standard library is used; `preflight` shells out to the
`codex` CLI if present. No dependency on any external skill or runtime.
"""
